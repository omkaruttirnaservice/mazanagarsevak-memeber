...
// import api, { BASE_URL } from "../utils/apiClient";
...
//   return (
//     <header className="fixed top-0 left-0 w-full z-80 bg-[#1c3c52] shadow-md">
...
//             >
//               {item}
//               <span className="absolute left-0 -bottom-1 w-0 h-0.5 bg-orange-800 group-hover:w-full transition-all duration-300"></span>
//             </Link>
//           ))}
//         </nav>
...
//   const isMemberRoute = location.pathname.startsWith("/member"); // ✅ Check if on member route

//   const imageSrc =
//     isMemberRoute && representative?.representativephoto
//       ? `${BASE_URL}/uploads/representative/${representative.representativephoto}`
//       : "/logo.jpeg"; // ✅ Conditional image based on route
            ...
            {navItems.map(({ name, path }) => (
              <Link
                ...
                className={`text-white text-base font-medium border-b pb-2 ${
                  location.pathname === path ? "text-orange-400" : "border-gray-600"
                }`}
                onClick={() => setMobileMenuOpen(false)}
              >
                {name}
